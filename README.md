...
This repository is an npm workspaces monorepo. v0.9.0 ships `packages/core` (a browser-safe session library), `apps/cli` (the terminal client), and `apps/web` (a static SvelteKit SPA with rooms, DMs, profile, unread counts, and per-room voice calls).
...
v0.9.0 is the daily-driver web UX (split rails, avatars, unread numbers, fenced code, notifications). v0.8.0 added per-room voice calls (mesh, STUN from `GET /ice`, no TURN). The full release plan lives in [hermes-be/docs/ROADMAP.md](https://github.com/ahyibrahim/hermes-be/blob/main/docs/ROADMAP.md).
